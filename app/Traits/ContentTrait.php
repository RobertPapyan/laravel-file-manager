...
trait ContentTrait
{
     ...
     */
    public function getContent($disk, $path = null,$chunk = 0): array
    {
    ...
    public function chunkContent($directories,$files,$chunk,$chunkSize){
        $allDirectoriesCount = sizeof($directories);
        $allFilesCount = sizeof($files);
        if($allDirectoriesCount >= $chunk*$chunkSize){
            $directories = array_chunk($directories,$chunkSize)[$chunk] ?? [];
            if(sizeof($directories) == $chunkSize){
                return [
                    'directories'=>$directories,
                    'files' => [],
                    'chunk' => ($chunk+1)*$chunkSize < $allDirectoriesCount+$allFilesCount ? $chunk : -1
                ];
            }
        }else{
            $directories = [];
        ...
        if(sizeof($directories)!= 0){
            $files = array_chunk($files,$chunkSize-sizeof($directories))[0];
        }else{
               $startIndex = ($allDirectoriesCount + $allFilesCount)- ($chunk)*$chunkSize;
               $files = array_slice($files,-$startIndex,$chunkSize);
        }
